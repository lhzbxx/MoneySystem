...
namespace Org\Util;
class Easemob {
	private $client_id='YXA6tlxWQFmoEeSzjQcMfprxOg';
	private $client_secret='YXA65TbuCVVfMchbtK-cWFbFTeq1xb4';
	private $org_name='aries';
	private $app_name='longge93';
	private $url;
    ...
    public function deleteFriend_must($owner_username, $friend_username){
        $s_1=$this->deleteFriend($owner_username, $friend_username);
        $options['username']=$friend_username;
        $options['password']='123456';
//        die($s_1."**********");
        if($s_1['status']!='200'){
            $this->openRegister($options);
            $s_2=$this->deleteFriend($owner_username, $friend_username);
        }
//        $this->deleteUser($friend_username);
        if($s_1['status']=='200' || $s_2['status']=='200'){
            return true;
        }
        else{
            return false;
    ...
    public function MultiOnline($user_arr){
        $num=count($user_arr);
        $urls=array();
        for($i=0;$i<$num;$i++){
            $urls[]=$this->url . "users/" . $user_arr[$i] . "/status";
        }
//        die(print_r($user_arr));
        ...
        foreach ($urls as $i => $url) {
            ...
            curl_setopt($conn[$i], CURLOPT_SSL_VERIFYPEER, FALSE ); // 对认证证书来源的检查
            curl_setopt($conn[$i], CURLOPT_SSL_VERIFYHOST, FALSE ); // 从证书中检查SSL加密算法是否存在
...
//        }
    ...
    function MessageSendCmd($from_user,$username_list,$content,$ext=''){
        $option ['target_type'] = "users";
        $option ['target'] = $username_list;
        $params ['type'] = "cmd";
...
//		$f=fopen("./temp/cmd2.txt","a+");
//		fwrite($f,print_r($option,true));
//		fclose($f);
    ...
    public function getUserAllGroup($username){
...
//        die($ql);
    ...
    public function download($url,$secret){
        //$this->url . "chatfiles/" . ;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $header [] = 'Accept: application/octet-stream';
        $header [] = 'share-secret: '.$secret;
        $result = $this->downloadCurl ( $url, '', $header, $type = "GET " );
        return $result;
    }
	/**
	 * 获取Token
...
//		pre ( $res );
    ...
    private function downloadCurl($url, $option, $header = 0, $type = 'POST') {
        $curl = curl_init (); // 启动一个CURL会话
        curl_setopt ( $curl, CURLOPT_URL, $url ); // 要访问的地址
        curl_setopt ( $curl, CURLOPT_SSL_VERIFYPEER, FALSE ); // 对认证证书来源的检查
        curl_setopt ( $curl, CURLOPT_SSL_VERIFYHOST, FALSE ); // 从证书中检查SSL加密算法是否存在
